...
from methylcoder import is_up_to_date_b, CPU_COUNT
...
def gmap_setup(gsnap_dir, out_dir, ref_fasta):
    ...
    if not (is_up_to_date_b(op.join(out_dir, make_path), cmd_last)):
        print >>sys.stderr, "C"
...
def run_gsnap(gsnap_dir, out_dir, ref_fasta, reads_fasta, cpu_count=CPU_COUNT):
    ...
    cmd += " -d %(ref_name)s --cmet %(reads_fasta)s > %(out_sam)s"
    cmd %= locals()
...
def fastq_to_gsnap_fasta(out_dir, reads):
    ...
    if is_up_to_date_b(reads[0], fa_name):
        return fa_name
...
def main(out_dir, ref_fasta, reads, gsnap_path):
    ...
    run_gsnap(gsnap_path, out_dir, ref_fasta, bsnap_fasta, cpu_count=CPU_COUNT)
